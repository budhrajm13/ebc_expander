class CommentAnalyzer {
  constructor() {
    this.testableCategories = {
      ...
      compatibility_issue: {
        phrases: ["wrong size", "too small", "too big"],
        confidence: 0.85,
      },
      material_difference: {
        phrases: [
  ...
  categorizeComment(text, analysis) {
    ...
    for (const [category, data] of Object.entries(this.testableCategories)) {
      const matchedPhrases = data.phrases.filter((phrase) =>
  ...
  calculateWordSentiment(words, index) {
    ...
    if (isPositive) {
      return hasNegation
        ? this.sentimentWeights.weights.negatedPositive
        : this.sentimentWeights.weights.positiveWord;
    }
    if (isNegative) {
      return hasNegation
        ? this.sentimentWeights.weights.negatedNegative
        : this.sentimentWeights.weights.negativeWord;
    }
    return 0;
  }
  ...
  checkForNegation(words, currentIndex) {
    const start = Math.max(
      0,
      currentIndex - this.sentimentWeights.negationDistance
    );
    const segment = words.slice(start, currentIndex);
    return segment.some((word) =>
      this.sentimentWeights.negationWords.includes(word)
    );
  }
  ...
  displayAnalysisResult(analysis, comment) {
    ...
    result += `- Sentiment Score: ${analysis.sentiment.score.toFixed(
      2
    )} (A score near -1 indicates strong negativity, near 1 indicates positivity)\n`;
    result += `- Raw Score: ${analysis.sentiment.details.rawScore.toFixed(
      2
    )} (This is the unadjusted score based on word sentiment)\n`;
    ...
    if (analysis.isTestable) {
      ...
      analysis.categories.forEach((category) => {
        result += `- **Category: ${category.name
          .replace("_", " ")
          .toUpperCase()}**\n`;
        result += `  - Confidence Level: ${(category.confidence * 100).toFixed(
          2
        )}% (This reflects how strongly the comment matches this category)\n`;
        result += `  - Matched Phrases: ${category.matchedPhrases.join(
          ", "
        )} (These phrases triggered the match for this category)\n\n`;
      });
    } else {
